...
## 14. 今後の予定：Logic Analyzer のアップデート

### 14.1 現状

`04-6502/web/index.html` は §9.2 の信号定義に基づくインライン `drawLA()` 関数（同ファイル 147 行目付近）で LA を描画している。これは 06-8080 から抽出・ライブラリ化された汎用 `RTLScopeLA`（[`js/rtlscope-la.js`](../../js/rtlscope-la.js)）とは独立した旧実装で、機能が大きく見劣りする。詳細は [`docs/91-rtlscope-la-LogicAnalyzerライブラリ.md`](../91-rtlscope-la-LogicAnalyzerライブラリ.md) を参照。

### 14.2 アップデート方針

| # | 作業 | 内容 |
| --- | --- | --- |
| 1 | **ライブラリ共通化** | インライン `drawLA()` を `RTLScopeLA` に置き換える。6502 は `ringWords=2`（Word0: AB/DB/WE/SYNC、Word1: A/X/Y/SP）で `update(head, heapu32, frozen, trigFireHead)` を RAF ループから呼ぶ。同期 Emscripten Module なので `Module.HEAPU32.subarray(ringBase, ringBase + RING_SIZE*2)` のビューを渡す（ゼロコピー） |
| 2 | **8080 版 LA 機能の移植** | zoom/pan（ホイール・ドラッグ）、マーカー A/B 差分、カーソル tooltip、信号トグル・並べ替え（localStorage 永続化）、T ステートグリッド、トリガーライン、DPR 対応など 8080 で確立した UI 機能を 6502 でも利用可能にする |
| 3 | **6502 サイクル デコードレーン** | 8080 の M-CYC レーンに相当する 6502 版を `decodeLane.render` コールバックで実装。`SYNC` パルスを起点に FETCH / READ / WRITE を色分け表示する |

### 14.3 移植時の注意

- 信号定義は §9.2 の `addr`(hex16) / `data` / `we` / `sync` を `RTLScopeLA` の信号定義オブジェクト形式（`{id,label,word,bit,type,width,color,...}`）に変換する
- `hex16` 相当はライブラリの `type:'hex'` + `width:16` で表現する
- `we=1` セグメントの色変え（Read/Write 区別）は `signalBackground` コールバックで注入する
- 本作業は docs/91 §8「今後の拡張予定」の "04-6502 への移行" / "6502 サイクル デコードレーン" に対応する
