...
static void print_status() {
    auto pc_region = [](uint16_t pc) -> const char* {
        if (pc < CPM_TPA_BASE)  return "page0";
        if (pc < CPM_CCP_BASE)  return "TPA";
        if (pc < CPM_BDOS_BASE) return "CCP";
        if (pc < CPM_BIOS_BASE) return "BDOS";
        return "BIOS";
    };
...
static int run_mode_boot_test(const char* bios_path, const char* cpm_path,
                               ...
                               const char* dsk_c_path, const char* dsk_d_path)
{
    ...
    char output[8192] = {};
    int  out_pos = 0;
    bool found = wait_for_prompt(output, out_pos, (int)sizeof(output), 10000000LL, 2000000LL);
    output[out_pos] = '\0';
...
// --exec <cmd>: CP/M 起動後にコマンドを実行して出力を表示する
static int run_mode_exec(const char* bios_path, const char* cpm_path,
                         const char* dsk_path,  const char* dsk_b_path,
                         const char* dsk_c_path, const char* dsk_d_path,
                         const char* exec_cmd)
{
    ...
    for (const char* p = exec_cmd; *p; p++) send_key((uint8_t)*p);
    send_key('\r');

    // コマンド実行・出力待ち（最大 2G ステップ、5M アイドルで打切り）
    // 完了検出: "Tests complete" または "Press a key"
    long long last_out_step = -1;
    ...
    for (long long s = 0; s < 2000000000LL && !tests_complete; s++) {
        ...
        while ((ch = get_display_char()) >= 0 && out_pos < (int)sizeof(output) - 1) {
            ...
            if ((out_pos >= 14 && strncmp(output + out_pos - 14, "Tests complete", 14) == 0) ||
                (out_pos >= 11 && strncmp(output + out_pos - 11, "Press a key",   11) == 0))
                tests_complete = true;
        }
        if (last_out_step >= 0) {
...
static int run_mode_run_test(const char* bin_path) {
...
// --bare-test <file>: バイナリを 0x0000 に配置してベアメタル実行する
static int run_mode_bare_test(const char* bin_path) {
    FILE* f = fopen(bin_path, "rb");
    if (!f) { fprintf(stderr, "cannot open: %s\n", bin_path); return 1; }
    fseek(f, 0, SEEK_END);
    long sz = ftell(f);
    ...
    // 最大 20G サイクル、アイドルタイムアウト 200M サイクル
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (ch < 0) break;
        char c = (char)(ch & 0x7F);
        if (out_pos < (int)sizeof(output) - 1) output[out_pos++] = c;
        if ((out_pos >= 11 && strncmp(output + out_pos - 11, "Press a key",   11) == 0) ||
            (out_pos >= 14 && strncmp(output + out_pos - 14, "Tests complete", 14) == 0))
            done = true;
    }
    output[out_pos] = '\0';
    fprintf(stdout, "%s\n", output);
    ...
    bool has_error = (strstr(output, "ERROR") != nullptr || strstr(output, "FAIL") != nullptr);
...
// インタラクティブ CP/M モードのメインループ
// ターミナル設定・シグナル登録・ディスクパス登録をここで行う。
static int run_mode_interactive(
    const char* bios_path, const char* cpm_path,
    const char* dsk_path,  const char* dsk_b_path,
    const char* dsk_c_path, const char* dsk_d_path,
    bool opt_no_save, bool opt_ro, bool opt_orig)
{
    ...
    // NOTE: sim_init 失敗時は restore_terminal() を呼んでいるが、
    //       この時点でまだ tcgetattr が未実行なので g_saved_termios は未初期化。
    //       安全のため tcgetattr を先に行っておく。
...
int main(int argc, char* argv[]) {
    ...
    if (boot_test)
        return run_mode_boot_test(bios_path, cpm_path, dsk_path,
    ...
    if (bare_test)
        return run_mode_bare_test(bare_test);

    if (run_test_bin) {
        if (run_test_default)
            fprintf(stdout, "run-test: %s\n", run_test_bin);
        return run_mode_run_test(run_test_bin);
    }

    return run_mode_interactive(bios_path, cpm_path, dsk_path,
                                dsk_b_path, dsk_c_path, dsk_d_path,
                                opt_no_save, opt_ro, opt_orig);
}
