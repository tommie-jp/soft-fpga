...
# LA_SIGNALS_ALL の全 id（index.html の定義順）
SIG_ALL = ['port', 'data', 'io_req', 'io_wr', 't_state', 'status', 'ir', 'f1', 'f2',
           'pc', 'sp', 'addr', 'dbin', 'sync', 'wr_n', 'hlda', 'wait', 'inte',
           'memr', 'memw', 'din', 'acc', 'reg_f', 'reg_b', 'reg_c', 'reg_d',
           'reg_e', 'reg_h', 'reg_l', 'dbus', 'mem1', 'mem2', 'mem3']  # 33 信号
...
class TimingCase:
    ...
    # MemWatch: {slot: addr} — 指定スロットに監視アドレスを設定（例: {0: 0x0350}）
    setup_mem_watch: dict[int, int] = field(default_factory=dict)
...
_C: list[TimingCase] = [
    ...
    # MEM1=0x0350 → MEMW=1 のクロックで RAM[0x0350] が 0x00→0xCD に変化する様子を観測
    TimingCase(29, 'mov_m_a',  0x77, 7, 2,
               [0x00, 0x77, 0x76],
               SIG_STD_MEM + ['mem1'], '16x', 34,
               setup_regs={'a': 0xCD, 'h': 0x03, 'l': 0x50},
               setup_mem={0x0350: [0x00]},
               setup_mem_watch={0: 0x0350},
...
_G: list[TimingCase] = [
    # PUSH B — 11T/3M  SP=0x0400 → 先に 0x03FF に B=0x12, 次に 0x03FE に C=0x34
...
_I: list[TimingCase] = [
    # SHLD $0350 — 16T/5M  HL を書く（L→0x0350, H→0x0351）
    TimingCase(51, 'shld_0350', 0x22, 16, 5,
               [0x00, 0x22, 0x50, 0x03, 0x76],
               SIG_STD_MEM + ['mem1', 'mem2'], '8x', 52,
               setup_regs={'h': 0x12, 'l': 0x34},
               setup_mem_watch={0: 0x0350, 1: 0x0351},
...
_J: list[TimingCase] = [
    # CALL $0350 — 17T/5M  0x0350 に HLT（SP=0x0400 → 0x03FF に PC_hi, 0x03FE に PC_lo を書く）
    TimingCase(53, 'call_0350', 0xCD, 17, 5,
               [0x00, 0xCD, 0x50, 0x03, 0x76],
               SIG_STD_MEM + ['mem1', 'mem2'], '8x', 54,
               setup_regs={'sp': 0x0400},
...
_K: list[TimingCase] = [
    ...
    # 書き込み順: M4で H→(SP+1)=0x03FF, M5で L→(SP)=0x03FE
    TimingCase(56, 'xthl',      0xE3, 18, 5,
               [0x00, 0xE3, 0x76],
               SIG_STD_MEM + ['mem1', 'mem2'], '8x', 56,
...
_N: list[TimingCase] = [
    # OUT port=01h — 10T/3M  A=0xAB を port 1 に出力（全信号表示）
    ...
    # IN port=01h — 10T/3M  port 1 から A に入力（全信号表示）
    TimingCase(68, 'in_port01',  0xDB, 10, 3,
               [0x00, 0xDB, 0x01, 0x76],
               SIG_ALL, '16x', 40,
               desc='Input from Port $01 to A'),

    # OUT port=FFh — 10T/3M  A=0xFF を port $FF に出力（全信号表示）
...
_P: list[TimingCase] = [
    # STAX B — 7T/2M  A=0xAB を (BC=0x0364) に書く
    # pre-context: LXI B, 0x0364 で BC をセット（setup_regs の代わりに命令で初期化）
    TimingCase(71, 'stax_b',    0x02, 7, 2,
               [0x01, 0x64, 0x03, 0x02, 0x76],  # LXI B,$0364 / STAX B / HLT
               SIG_STD_MEM + ['mem1'], '16x', 34,
               setup_regs={'a': 0xAB},
               setup_mem_watch={0: 0x0364},
...
def _run_case(sim: SimAPI, tc: TimingCase) -> None:
    ...
    # 5. MemWatch 設定（MEMW 観測用アドレスをスロットに設定）
    #    全スロットを一旦クリアしてから tc.setup_mem_watch で上書きする。
    for slot in range(3):
        if slot in tc.setup_mem_watch:
            sim.set_mem_watch(slot, tc.setup_mem_watch[slot])
        else:
            sim.clear_mem_watch(slot)

    # 6. レジスタ設定（F は set_regs 不可 → code 内の命令で設定）
    if tc.setup_regs:
        sim.set_regs(**tc.setup_regs)

    # 7. テストコードを 0x0300 に書き込む
    sim.write_mem(0x0300, tc.code)

    # 8. 追加メモリ設定（オペランド・スタック・割り込みベクタ等）
    for addr, data in tc.setup_mem.items():
        sim.write_mem(addr, data)

    # 9. トリガー設定（テスト命令の M1 フェッチで発火）
    #    clear_trigger() → reset_trigger_state() により ring_frozen も解除される。
    #    pc フィルタで BIOS や プリコンテキスト NOP (0x0300) での誤発火を防ぐ。
    #    tc.code[0] は常に pre-context NOP のため index=1 以降で opc を探す。
    #    プリアンブルがある場合（例: jp_taken は 0x0303）は正しく計算される。
    try:
        opc_offset = tc.code.index(tc.opc, 1)
    except ValueError:
        opc_offset = 1  # フォールバック
    instr_pc = 0x0300 + opc_offset
    sim.clear_trigger()
    sim.set_trigger(type='instr', pc=instr_pc, opc=tc.opc)
    ...
    # 11. トリガー発火待ち（la._trigHead が更新されるのを 30 秒間ポーリング）
    sim.await_wait_trigger(30_000)

    # 12. 停止・安定待ち（post_delay 分の ring buffer 収集が完了するまで）
    sim.pause()
    time.sleep(0.15)

    # 13. LA 表示設定（全レジスタ + 命令固有信号）
...
def test_timing_screenshot(sim: SimAPI, tc: TimingCase) -> None:
    ...
    # ── チェック 2: タイミング図簡易正当性検証 ─────────────────────────────────
    #
    # 根拠:
    #   - トリガーは type='instr', opc=tc.opc で M1 FETCH に発火する
    #   - offset=0  : M1 T1（SYNC）→ addr = 命令フェッチアドレス（テストコード内のはず）
    #   - offset=2  : M1 T2（DBIN=1, MEMR=1）→ データバス = フェッチしたオペコード
    #
    # 検出できる失敗パターン:
    #   - BIOS/CCP 内の同一オペコードでトリガーが誤発火 → addr が 0x0300-0x03FF 外
    #   - 期待外の命令が実行された               → dbus ≠ tc.opc
    #   - シミュレーションがリセット状態のまま     → addr=0x0000, dbus=0x00

    addr_val = sim.read_sample('addr', 0)
    addr_str = f'0x{addr_val:04X}' if addr_val is not None else 'None'
    assert addr_val is not None and 0x0300 <= addr_val <= 0x03FF, (
        f"[{tc.seq:02d} {tc.mnemonic}] "
        f"T=0 addr={addr_str} (期待: 0x0300–0x03FF) "
        f"— テストコード外でトリガーが発火した可能性"
    )

    dbus_val = sim.read_sample('dbus', 2)
    assert dbus_val == tc.opc, (
        f"[{tc.seq:02d} {tc.mnemonic}] "
        f"M1 T2 dbus=0x{dbus_val:02X} (期待: 0x{tc.opc:02X}) "
        f"— 期待外のオペコードがフェッチされた"
    )
