# PDP-11 Verilog 実装の調査メモ <a class="qr-link" href="../../../docs/09-PDP11/01-PDP11-verilogコア調査-QR.png">QR</a>
...
#### EIS クロック数実測（WASM ベアメタルモード）

`verif/bench_eis.mjs` による実測（bare metal WASM、ring head でサイクル計測）：

| 命令 | EIS 実行 clk | ソフトウェア実装 clk | 高速化率 |
|------|-------------|---------------------|---------|
| `MUL r1, r0` (5×3) | 24 | 450（shift-add 16 bit × 16 bit） | **18.8×** |
| `ASH r1, r0` (<<7) | 15 | 91（`asl` × 7 回ループ） | **6.1×** |
| `DIV r2, r0` (13÷5) | 47 | 91（引き算ループ） | **1.9×** |

> DIV のソフトウェア比較は引き算ループ（商 2 を得るまで 2 回の減算）で、
> 実際の 16 bit 除算アルゴリズム（shift + subtract × 16）より大幅に速い。
> 汎用実装では EIS DIV の優位性はさらに大きくなる。
